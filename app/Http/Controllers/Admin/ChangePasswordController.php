    ...
    public function update(Request $request)
    {
        ...
        if(md5($request->password) == Auth::user()->password){
            ...
            if($user){
                ...
                return redirect()->route('admin.profil.edit', Auth::user()->id)->with(['success' => 'Password berhasil diganti!']);
            } else {

                return redirect()->route('admin.profil.edit', Auth::user()->id)->with(['error' => 'Password gagal diganti!']);
            }
        } else {

            return redirect()->route('admin.profil.edit', Auth::user()->id)->with(['error' => 'Password salah!']);
        }
    }
}