            ...
            <table class="w-full table table-striped">
                <thead class="table-dark">
                    ...
                    <tr class="text-xs font-semibold tracking-wide text-center text-white uppercase border-b dark:border-gray-700 dark:text-gray-400 dark:bg-gray-800">
                        @for($i=5; $i<=18; $i++) <th class="whitespace-nowrap text-center">{{$i+1}}</th>
                            @endfor
                    </tr>
                </thead>
...
<script>
    $('#pilihSite').on('change', function() {
        ...
        $i.ajax({
            type: "POST",
            url: 'http://ptrci.co.id/datacenter/public/super_admin/productivity_filter?layout=side-menu',
            data: {
                'kodesite': kodesite,
            ...
            success: function(result) {
                ...
                fullText = ""
                if (result) {
                    $i.each(result.data, function(index, element) {
                        var number = index + 1
                        text = '<tr class="text-center bg-white">' +
                            ...
                            '<td class="">' + result.data[index].avg_pty + '</td>' +
