    ...
    <div class="grid grid-cols-12 gap-5">
        ...
        <div class="flex justify-between items-center col-span-12 mt-5">
            ...
            <div class="ml-auto flex justify-center items-center">
                ...
                {{-- Url Rujukan --}}
                <input type="hidden" name="url" value="{{route('mp.index')}}" id="urlFilter">
                ...
                <select id="status_karyawan"
                    ...
                    @foreach ($status_karyawan as $st)
                        <option value="{{ $st->statuskary }}">{{ $st->statuskary }}</option>
                    @endforeach
                </select>

                
                {{-- Pilih Field Cari --}}
                <select id="field_cari"
                    class="block shadow-sm p-2 mr-2 rounded-md w-20  text-sm dark:text-gray-300 dark-gray-600 dark:bg-gray-700 form-multiselect focus-stone-400 focus:outline-none focus:shadow-outline-stone dark:focus:shadow-outline-gray"
                    name="field_cari" id="field_cari">
                    @foreach ($field_cari as $st)
