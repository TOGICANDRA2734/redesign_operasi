...
<div class="intro-y col-span-12">
    <div class="grid grid-cols-1 gap-3">
        <!-- Start Form -->
        <form action="{{route('hm.store')}}" method="POST" id="storeDok" class="px-4 py-3 mb-8 grid grid-cols-2 gap-5 bg-white rounded-lg  dark:bg-gray-800">
            @csrf
            ...
            <div>
                <label class="block mt-1 text-sm">
                    ...
                    <select data-placeholder="Pilih site" name="kodesite" class="w-full @error('kodesite') border-danger @enderror" aria-readonly>
                        <option value="{{ Auth::user()->kodesite}}" selected>{{\App\Models\Site::select('namasite')->where('kodesite', Auth::user()->kodesite)->pluck('namasite')[0]}}</option>
                    </select> 
                </label>
            </div>
            ...
            <div>
                ...
                <input id="crud-form-1" class="form-control w-full" type="number" step="0.01" min="0" name="hm" id="hm" placeholder="Input HM">
                @error('hm')
                    <div class="text-danger mt-2">{{$message}}</div>
                @endif
            </div>
            ...
            <div>
                ...
                <input id="crud-form-1" class="form-control w-full" type="number" step="0.01" min="0" name="km" id="km" placeholder="Input KM">
                @error('hm')
                    <div class="text-danger mt-2">{{$message}}</div>
                @endif
            </div>
